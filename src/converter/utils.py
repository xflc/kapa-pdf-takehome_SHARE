...
def get_block_prompt(block_type: str, original_text_context: str | None = None, toc_text: str | None = None) -> str:
    ...
    toc_reference = (
        "Use the corresponding markdown heading ('3.' -> '##', '2.3' -> '###', or '5.6.6' -> '####', etc.)\n If there is no number, dont use any '#' at all." 
        if not toc_text 
        else f"\n\nFormat this heading as a markdown section heading. If it starts with numbers, use the corresponding level of markdown heading ('3.' -> '##', '2.3' -> '###', or '5.6.6' -> '####', etc.). If there is no number, check the Table of Contents of the document, to help you clarify how many '#' you should use if any. Things like legends, footnotes, captions, etc. should have no '#' at all, just unformatted text:\n\n{toc_text}\n\n"
    )
    prompts = {
        ...
        "SectionHeader": f"Extract the section heading from the attached png with the appropriate markdown heading '#' level if applicable.{toc_reference}\nJust output the exact content of the attached image with the appropriate markdown heading '#' level if applicable (Use the ```markdown``` tags to wrap the markdown) and nothing else.",
