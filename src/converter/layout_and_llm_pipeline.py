...
class LayoutAndLLMConverter(PDFtoMarkdown):
    ...
    def _pdf_to_markdown_pipeline(self, pdf_path: Path) -> str:
        ...
        toc_texts = []
        for page_num, (layout_result, page_image) in enumerate(
            ...
            for block in layout_result.bboxes:
                if block.label == "TableOfContents":
                    toc_text = self._lllm_extract_text_from_block(
                        ...
                        page_image,
                        layout_size,
                        original_text_contexts[page_num],
                        toc_text=None,
                    )
                    if toc_text:
        ...
        page_texts = self._process_all_pages(
            layout_results, images, original_text_contexts, toc_text=combined_toc_text
        )
    ...
    def _process_all_pages(
        ...
        original_text_contexts: List[str],
        toc_text: str = None,
    ) -> List[str]:
        """
        ...
        with tqdm(total=len(layout_results), desc="Processing pages") as page_pbar:
            ...
            ) in enumerate(zip(layout_results, images, original_text_contexts)):
    ...
    def _lllm_extract_text_from_block(
        self,
        block,
        page_image: Image.Image,
        layout_size,
        ...
        if block_type in {"Table"} and main_content:
            ...
            legend_response = completions_with_backoff(
                ...
                messages=get_user_messages(
                    img_base64,
                    legend_prompt,
                    system_prompt="You are a helpful assistant that generates a readable representation of a table",
                ),
                temperature=0.2,
            )
            legend_text = legend_response.choices[0].message.content
            legend_content = (
                extract_markdown_content(legend_text) if legend_text else ""
            )
            # If the combined length exceeds max_chars, return only legend_content
            if (
                legend_content
                and len(main_content) + len(legend_content) > self.max_chars
            ):
                return (
                    "\n\n" + legend_content + "\n\n"
                    if len(legend_content) < self.max_chars
                    or len(main_content) > self.max_chars
                    else "\n\n" + main_content + "\n\n"
                )
