...
from src.converter.openai import completions_with_backoff
from src.converter.utils import extract_markdown_content, visualize_layout_blocks
import structlog
import numpy as np
import logging
import structlog
...
if __name__ == "__main__":
    # Example 1: Just inspect layout (no API calls)
    print("=== Layout Inspection Example ===")
    pdf_file = "data/pdfs/your_file.pdf"  # Update this path
    
    # Example 2: Full processing with text extraction
    if os.getenv("OPENAI_API_KEY"):
        print("\n=== Full Processing Example ===")
        try:
            # Enable debug visualization to see detected blocks
            markdown_result = process_pdf_simple(pdf_file, debug_visualize=True)
            print(f"Conversion complete! Result length: {len(markdown_result)} characters")
        ...
        print("\nSet OPENAI_API_KEY to run full processing example") 